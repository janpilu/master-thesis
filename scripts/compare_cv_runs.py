...
def perform_statistical_tests(df1, df2, metric):
    ...
    for config in configs:
        ...
        if len(common_folds) < len(data1_by_fold) or len(common_folds) < len(data2_by_fold):
            print(f"⚠️ Warning: Config {config} has different folds between the two folders.")
...
def create_comparison_matrix(results_by_metric, output_dir, folder1_name, folder2_name, first_best_epochs):
    """
    ...
    fixed_title = "10-fold Cross Validation Comparison: Fine-tuned versus frozen encoder"
    ...
    measures = {
        'p_value': {'title': 'Wilcoxon P-Value (Lower is Better)', 'cmap': 'Reds_r', 'vmin': 0, 'vmax': 0.1},
        'cohens_d': {'title': 'Cohen\'s d (Higher is Better)', 'cmap': 'Reds', 'vmin': 0, 'vmax': 2.0},
        'difference': {'title': 'Absolute Difference', 'cmap': 'Reds', 'vmin': 0, 'vmax': None}
    }
            ...
            for j, config in enumerate(all_configs):
                ...
                if measure == 'p_value':
                    ...
                    if props['vmax'] is None:
                        if i == 0:  # First metric
                            props['vmax'] = max(abs(results[f'{metric}_diff'].max()), 0.001)
                        else:
                            props['vmax'] = max(props['vmax'], abs(results[f'{metric}_diff'].max()))
    ...
    for i, metric in enumerate(metrics):
        ...
        for j, config in enumerate(all_configs):
            ...
            if metric == 'val_loss':  # For loss, lower is better
                better = folder2_name if row[f'{metric}_diff'].values[0] > 0 else folder1_name
            else:
                better = folder1_name if row[f'{metric}_diff'].values[0] > 0 else folder2_name
            
            # Check if difference is significant
            is_significant = row['wilcoxon_significant'].values[0] or row['t_test_significant'].values[0]
...
def main():
    ...
    second_folder_name = questionary.select(
        "Select second CV folder:",
        choices=folder_names,
        style=custom_style
    ).ask()
    
    # Find the full paths of the selected folders
    first_folder = next((folder for folder in cv_folders if os.path.basename(folder) == first_folder_name), None)
    second_folder = next((folder for folder in cv_folders if os.path.basename(folder) == second_folder_name), None)
    ...
    for metric in metrics:
        ...
        results = perform_statistical_tests(first_best_epochs, second_best_epochs, metric)
        ...
        if save_result:
            ...
            output_filename = f"comparison_{first_folder_name}_vs_{second_folder_name}_{metric}.csv"
    ...
    save_combined = questionary.confirm(
        "Do you want to save a combined summary report?",
        style=custom_style
    ).ask()
    ...
    if save_combined:
        ...
        for metric in metrics:
            ...
            for _, row in results.iterrows():
                config = row['config']
                better = first_folder_name if row[f'{metric}_diff'] > 0 else second_folder_name
                if metric == 'val_loss':  # For loss, lower is better
                    better = second_folder_name if row[f'{metric}_diff'] > 0 else first_folder_name
                ...
                summary_data.append({
                    ...
                    'significant': row['wilcoxon_significant'] or row['t_test_significant'],
                    'cohens_d': row['cohens_d'],
                    'effect_size': row['effect_size_interpretation']
                })
        
        summary_df = pd.DataFrame(summary_data)
        summary_filename = f"comparison_summary_{first_folder_name}_vs_{second_folder_name}.csv"
        ...
        report_path = os.path.join(comparison_dir, f"comparison_report_{first_folder_name}_vs_{second_folder_name}.txt")
        
        with open(report_path, 'w') as f:
            f.write(f"Statistical Comparison: {first_folder_name} vs {second_folder_name}\n")
            f.write("=" * 80 + "\n\n")
            ...
            if len(significant_findings) > 0:
                for _, row in significant_findings.iterrows():
                    ...
                    f.write(f"- Config {row['config']}, {row['metric']}: {row['better_model']} is better ")
            ...
            for metric in metrics:
                ...
                for _, row in metric_data.iterrows():
                    ...
                    f.write(f"- Config {row['config']}: {row['better_model']} is better ")
                    f.write(f"(diff: {row['difference']:.4f}, p-value: {p_value_str}, ")
    ...
    results_by_metric = {metric: perform_statistical_tests(first_best_epochs, second_best_epochs, metric) for metric in metrics}
    
    # Create comparison matrix
    create_comparison_matrix(results_by_metric, comparison_dir, first_folder_name, second_folder_name, first_best_epochs)
